...
(function () {
    ...
    function init() {
        ...
        style.textContent = `
            ...
            #teams-list { width: 100%; justify-content: center; text-align: center; margin-top: 5px; }
            ...
            .player-avatar { width: 54px; height: 54px; flex: 0 0 54px; margin: 1px auto 8px; border-radius: 8px; border: 1px solid #292929; background: #050505; box-shadow: none; }
            .player-card h3 { margin: 0; font-size: 13px; line-height: 1.05; font-weight: 800; letter-spacing: -.015em; }
            .player-role { margin-top: 3px; padding: 3px 7px; border-radius: 4px; font-size: 8px; line-height: 1; font-weight: 800; }
            .player-country { margin-top: 2px; font-size: 8px; line-height: 1.05; font-weight: 700; }
            ...
            #loginModal #loginForm { display: flex !important; flex-direction: column !important; gap: 9px !important; align-items: stretch !important; }
            #loginModal #loginForm label { display: flex !important; flex-direction: column !important; margin: 0 !important; font-size: 9px !important; font-weight: 800; text-transform: uppercase; letter-spacing: .05em; color: #7f8791; }
            #loginModal #loginForm input { width: 100%; height: 36px; margin-top: 5px; padding: 0 10px; border-radius: 7px; font-size: 12px; box-sizing: border-box; }
            ...
            #playerPage .player-profile-actions a, #playerPage .player-profile-actions button, #playerPage .player-profile-actions .primary, #playerPage .player-profile-actions .secondary, #playerPage .player-profile-actions .edit-btn { appearance: none !important; display: inline-flex !important; align-items: center; justify-content: center; min-height: 38px; padding: 9px 14px !important; border: 1px solid #2b2b2b !important; border-radius: 8px !important; background: #0d0d0d !important; color: #d7d7d7 !important; text-decoration: none !important; font-size: 10px; font-weight: 800; line-height: 1; cursor: pointer; box-shadow: none !important; transition: transform .18s ease, background .2s ease, border-color .2s ease, color .2s ease, box-shadow .2s ease; }
            #playerPage .player-profile-actions a:hover, #playerPage .player-profile-actions button:hover, #playerPage .player-profile-actions .primary:hover, #playerPage .player-profile-actions .secondary:hover, #playerPage .player-profile-actions .edit-btn:hover { background: #171717 !important; border-color: #444 !important; color: #fff !important; transform: translateY(-2px); box-shadow: 0 10px 28px rgba(0,0,0,.28) !important; }
            #playerPage .player-profile-actions .danger, #playerPage .player-profile-actions .remove-btn, #playerPage .player-profile-actions [class*="danger"] { background: #120909 !important; border-color: rgba(210,60,60,.38) !important; color: #ff8b8b !important; }
            ...
            @media (max-width: 760px) {
                ...
                #playerPage .player-profile-actions a, #playerPage .player-profile-actions button { flex: 1 1 145px; }
                #loginModal .login-card { width: min(420px, calc(100vw - 24px)) !important; padding: 20px 16px !important; }
            }
            @media (max-width: 430px) { .player-grid { grid-template-columns: 1fr; } #teams .hero h1 { font-size: 41px; } }
        `;
        ...
        function spaceSubstitutes() {
            ...
            page.querySelectorAll("*").forEach(el => {
                ...
                if (text === "SUBSTITUTES" || text === "ЗАМЕНЫ") {
                    if (text === "SUBSTITUTES") el.textContent = "ЗАМЕНА";
        ...
        function showPage(target) {
            ...
            if (target === "teams") document.getElementById("teams-list")?.scrollIntoView({ behavior: "smooth", block: "start" });
